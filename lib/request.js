...
function buildRequest (opts) {
  ...
  if (isHttp2) {
    ...
    // we allow passing undici as module. If the user passes
    // an invalid constructor the behaviour will be undefined
    // or throw. We allow passing the of the properties on
    // opts.undici as options
    if (opts.undici.Pool) {
      pool = new opts.undici.Pool(baseUrl, opts.undici)
    } else {
